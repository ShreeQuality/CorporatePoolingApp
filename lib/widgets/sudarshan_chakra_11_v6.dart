...
/// Screen 1 Emblem: Sacred Animated Sudarshan Chakra (V11-V6: Multi-Layer Polyphonic Kinetic Resonance)
/// Pattern:
///   - Independent Layer Physics: Each layer vibrates with its own distinct frequency, phase & mass:
///       * Outermost Perimeter Ring + 36 Teeth: Heavy inertial orbital pulse (1.2Hz sinusoidal heave)
///       * Dashed Arc Circle (- - -): Rapid counter-directional shimmer (16Hz micro-sparkle)
///       * Main Sacred Wheel (24 Spokes & Hub): Deep foundational gyroscopic rumble (3.6Hz orbit)
///       * Outer Fire Star: Rhythmic radial flare breathing (2.2Hz, +3.5% scale)
///       * Inner Fire Star: High-energy electric plasma tremor (24Hz jitter)
...
class SudarshanChakra11V6 extends StatefulWidget {
  const SudarshanChakra11V6({
    super.key,
    this.size = 170,
    this.speed = 1.0,
  });
  ...
  @override
  State<SudarshanChakra11V6> createState() => _SudarshanChakra11V6State();
}

class _SudarshanChakra11V6State extends State<SudarshanChakra11V6>
  ...
  late final AnimationController _polyphonicTimeController;
  ...
  void initState() {
    ...
    // 7. Polyphonic Multi-Layer Time Base (3000ms)
    _polyphonicTimeController = AnimationController(
      vsync: this,
  ...
  void dispose() {
    ...
    _polyphonicTimeController.dispose();
    super.dispose();
  }
      ...
      child: Center(
        child: AnimatedBuilder(
          animation: Listenable.merge([
            ...
            _glowController,
            _polyphonicTimeController,
          ]),
          builder: (context, _) {
            final t = _polyphonicTimeController.value * 3.0; // t in seconds (0.0 to 3.0)

            // LAYER 0: Outermost 36-Teeth Perimeter (Heavy 1.2Hz orbital heave)
            final outerX = math.sin(t * 1.2 * math.pi * 2) * 1.8;
            final outerY = math.cos(t * 1.2 * math.pi * 2) * 1.4;

            // LAYER 1: Main Sacred Wheel (Foundational 3.6Hz gyro rumble)
            final wheelX = math.sin(t * 3.6 * math.pi * 2) * 1.2;
            final wheelY = math.cos(t * 3.6 * math.pi * 2) * 0.9;

            // LAYER 1B: Dashed Circle Line (Rapid 16Hz counter-shimmer)
            final dashedX = math.cos(t * 16.0 * math.pi * 2) * 1.4;
            final dashedY = math.sin(t * 16.0 * math.pi * 2) * 1.0;

            // LAYER 2: Outer Fire Star (2.2Hz radial flare breathing)
            final fireOuterScale = 1.0 + (0.035 * math.sin(t * 2.2 * math.pi * 2));
            final fireOuterX = math.sin(t * 2.2 * math.pi * 2) * 0.8;
            final fireOuterY = math.cos(t * 2.2 * math.pi * 2) * 0.8;

            // LAYER 3: Inner Fire Star (24Hz electric plasma tremor)
            final plasmaTremorX = math.sin(t * 24.0 * math.pi * 2) * 1.5;
            final plasmaTremorY = math.cos(t * 24.0 * math.pi * 2) * 1.2;
            ...
            return Stack(
              ...
              children: [
                ...
                // 2. 3D Tilt Container with Polyphonic Multi-Layer Resonances
                Transform(
                  alignment: Alignment.center,
                  transform: Matrix4.identity()
                    ..scale(1.60, 1.60, 1.0)
                  ...
                  child: Stack(
                    alignment: Alignment.center,
                    children: [
                      // LAYER 0: Outermost Perimeter Circle with 36 Blade Teeth (10s CW + 1.2Hz heave)
                      Transform(
                        alignment: Alignment.center,
                        transform: Matrix4.identity()..translate(outerX, outerY, 0.0),
                      ...
                      // LAYER 1: Main Sacred Wheel (8s CW + 3.6Hz gyro rumble)
                      Transform(
                        alignment: Alignment.center,
                      ...
                      // LAYER 1B: Dashed Circle Line (- - -) (6.5s CCW + 16Hz shimmer)
                      Transform(
                        alignment: Alignment.center,
                      ...
                      // LAYER 2: Outer Fire Star (5.5s CW + 2.2Hz radial flare breathing)
                      Transform(
                        alignment: Alignment.center,
                        transform: Matrix4.identity()
                      ...
                      // LAYER 3: Inner Fire Star (4.5s CCW + 24Hz electric plasma tremor)
                      Transform(
                        alignment: Alignment.center,
                        transform: Matrix4.identity()
                          ..translate(plasmaTremorX, plasmaTremorY, 0.0),
                        child: Transform.rotate(

# ðŸ§  SPRINT 2: MASTER MEMORY & ARCHITECTURAL SOURCE OF TRUTH
...
Sprint 2 establishes the primary foundation of the KarmaRide mobile application after authentication (Sprint 1). It provides a persistent, state-preserving **5-Tab Navigation Shell**, the **Dual-Mode Commute Dashboard** (Find a Ride vs. Give a Ride), the **Wallet & ESG Banner**, the **Personal Safety Hub**, and the **User Profile Management Suite**.

```
+-----------------------------------------------------------------------------------------------+
|                                  MAIN APPLICATION SHELL                                       |
|                                                                                               |
|  [ Tab 1: Home ]  |  [ Tab 2: My Rides ]  |  [ Tab 3: Wallet ]  |  [ Tab 4: Safety ]  | [ Tab 5: Profile ]  |
+-----------------------------------------------------------------------------------------------+
```

---

## 2. Master Navigation Shell (5 Primary Tabs)

The core shell (`lib/screens/home/home_shell_screen.dart`) manages a custom transparent floating bottom navigation bar wrapping an `IndexedStack` to ensure **zero state loss** during tab switching.

### ðŸ  Tab 1: Home / Commute Dashboard (`tabs/dashboard_tab.dart`)
* **Top Header:** Greeting ("Good morning, [Name]"), Corporate Verification Badge (`[ ðŸ¢ Infosys ]` or `[ ðŸŒ Public Commuter ]`), Unread Notification Bell, and Logout CTA.
* **`WalletSummaryBanner` (SRS Â§12.4):** Top glassmorphic dashboard widget displaying:
  ...
  3. **Lifetime COâ‚‚ Saved ($kg$):** Real-time SEBI Scope 3 ESG carbon accounting ($0.15\text{ kg CO}_2\text{/km}$).
  4. **Safety Trust Score Badge ($0â€“100$):** Double-blind peer review score with colored status aura.
...
* **Dual Commute Mode Switcher:** Top segmented toggle between:
  * ðŸ” **Find a Ride (Rider Mode)**
  * ðŸš— **Give a Ride (Driver Mode)**

---

### ðŸš— Tab 2: "My Rides" & Commute Roster (`tabs/my_rides_tab.dart` â€” SRS Â§19)
...
| :--- | :--- | :--- |
| **1. ðŸŸ¡ ACTIVE** | `started`, `driver_en_route`, `arrived_at_pickup` | Live GPS map preview, real-time 5-minute arrival countdown timer, and 1-Tap Boarding PIN/BLE trigger. |
| **2. ðŸ”µ UPCOMING** | `published`, `requested`, `accepted` | Confirmed bookings for today & tomorrow, companion profiles, seat counts, and "Cancel Commute" option. |
| **3. ðŸ” RECURRING** | `scheduled_recurring` (Monâ€“Fri) | Monthly commute calendar roster, "Skip Today" button, and Vacation Pause Mode toggle. |
| **4. ðŸ“œ PAST** | `completed`, `cancelled` | Historic breadcrumb GPS route replay, transaction cost breakdown, and **Downloadable PDF SEBI ESG Carbon Certificate**. |

---

### ðŸª™ Tab 3: Wallet, Double-Entry Ledger & ESG (`tabs/wallet_tab.dart` â€” SRS Â§12)
...
* **1-Tap Colleague Transfer CTA (SRS Â§12.7):** Send thank-you coins to pool partners with a personalized note.
* **Double-Entry Financial Ledger:** Real-time audit trail displaying `+` Green credit earnings and `-` Red ride payments.

---

### ðŸ›¡ï¸ Tab 4: Safety Shield & Emergency Center (`tabs/safety_tab.dart` â€” SRS Â§11)
...
### ðŸ‘¤ Tab 5: User Profile & Identity (`tabs/profile_tab.dart` â€” SRS Module H)
...
```
+-----------------------------------------------------------------------------------------------+
|                       [ ðŸ” Find a Ride (Rider) ]  |  [ ðŸš— Give a Ride (Driver) ]              |
+-----------------------------------------------------------------------------------------------+
```

### ðŸ” A. "Find a Ride" (Rider Mode Panel)
...
* **CTA Button:** `"Find rides near me"` â€” triggers spatial search into Sprint 4.

### ðŸš— B. "Give a Ride" (Driver Mode Panel)
...
* **Recurring Commute Toggle:** 1-Tap toggle to repeat Monâ€“Fri.
* **CTA Button:** `"Post my route"` â€” publishes ride into Sprint 3.
...
lib/screens/home/widgets/
 â”œâ”€â”€ wallet_summary_banner.dart   <-- Coins, Corporate Grant, CO2 & Trust Score
 â”œâ”€â”€ quick_commute_card.dart      <-- Time-aware 1-Tap predictive commute suggestion
 â””â”€â”€ driver_upgrade_card.dart     <-- CTA for commuters who skipped Driver KYC
```
...
* **Glass Panels (`GlassPanel`):**
  ...
  * `1.0px` hairline accent borders:
    * ðŸŸ¡ Gold (`0xFFFFB74D`): Karma Coins & Financials.
    * ðŸ”µ Cyan (`0xFF00E5FF`): Corporate Badges & Identity.
    * ðŸŸ¢ Emerald (`0xFF00E676`): Driver Mode & Eco COâ‚‚.
    * ðŸ”´ Coral (`0xFFFF5252`): Safety SOS & Alerts.
* **Haptics:**

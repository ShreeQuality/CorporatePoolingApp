...
/// Screen 1 Emblem: Sacred Animated Sudarshan Chakra (V11-V5: Staccato Tick / Precision Chronometer Escapement)
/// Pattern:
///   - High-Precision Mechanical Ratchet: 4 ultra-crisp micro-ticks per second (250ms interval)
///   - Instantaneous 40ms impulse snap with rigid dead-stop damping (zero lag / crisp Swiss mechanical gear feel)
...
class SudarshanChakra11V5 extends StatefulWidget {
  const SudarshanChakra11V5({
    super.key,
    this.size = 170,
    this.speed = 1.0,
  });
  ...
  @override
  State<SudarshanChakra11V5> createState() => _SudarshanChakra11V5State();
}

class _SudarshanChakra11V5State extends State<SudarshanChakra11V5>
  ...
  late final AnimationController _staccatoTickController;
  ...
  void initState() {
    ...
    // 7. Staccato Tick Cycle: 1000ms (contains 4 precision ticks at 0ms, 250ms, 500ms, 750ms)
    _staccatoTickController = AnimationController(
      vsync: this,
  ...
  void dispose() {
    ...
    _staccatoTickController.dispose();
    super.dispose();
  }
      ...
      child: Center(
        child: AnimatedBuilder(
          animation: Listenable.merge([
            ...
            _glowController,
            _staccatoTickController,
          ]),
          builder: (context, _) {
            final t = _staccatoTickController.value;

            // 4 ticks per cycle (every 0.25 in normalized time, corresponding to 250ms)
            final subPhase = (t * 4) % 1.0; // 0.0 -> 1.0 within each 250ms tick slot

            double tickImpulse = 0.0;
            double tickScale = 1.0;
            double tickAngle = 0.0;

            // Crisp 40ms snap impulse (subPhase < 0.16)
            if (subPhase < 0.16) {
              final p = subPhase / 0.16; // 0.0 -> 1.0 within the 40ms impulse
              final pulse = math.sin(p * math.pi); // Sharp half-sine spike
              tickImpulse = pulse * 1.8;
              tickScale = 1.0 + (0.016 * pulse);
              tickAngle = (pulse * 0.008); // Micro mechanical escapement twitch
            }

            final baseGlow = _glowController.value;
            final glowScale = 1.62 + (baseGlow * 0.08) + (tickImpulse * 0.04);
            ...
            return Stack(
              ...
              children: [
                ...
                // 2. 3D Tilt Container with Precision Escapement Ratchet Tick
                Transform(
                  alignment: Alignment.center,
                  transform: Matrix4.identity()
                    ..scale(1.60 * tickScale, 1.60 * tickScale, 1.0)
                    ..rotateX(-54 * math.pi / 180)
                    ..rotateZ((10 * math.pi / 180) + tickAngle),
                  child: Stack(
                    alignment: Alignment.center,
                    children: [
                      ...
                      Transform(
                        alignment: Alignment.center,
                        transform: Matrix4.identity()
                          ..translate(tickImpulse * 0.8, -tickImpulse * 0.4, 0.0),
                      ...
                      Transform(
                        alignment: Alignment.center,
                        transform: Matrix4.identity()
                          ..translate(-tickImpulse * 0.5, tickImpulse * 0.5, 0.0),
                        child: Transform.rotate(
                      ...
                      Transform(
                        alignment: Alignment.center,
                        transform: Matrix4.identity()
                          ..translate(tickImpulse * 0.6, tickImpulse * 0.6, 0.0),
                        child: Transform.rotate(

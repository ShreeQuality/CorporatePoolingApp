...
/// Screen 1 Emblem: Sacred Animated Sudarshan Chakra (V11-V7: Multi-Layer Differential Depth / Fast Core + Slow Ring)
/// Pattern:
///   - Deepest 3D Layered Parallax Separation:
///       * Outermost Perimeter Ring + 36 Teeth: Slow, massive cosmic rim (14s CW) + 0.7Hz slow gravitational drift
///       * Dashed Arc Circle (- - -): Counter buffer ring (7.5s CCW)
///       * Main Sacred Wheel (24 Spokes & Hub): Medium momentum driver (8s CW)
///       * Outer Fire Star: Accelerated energy spin (3.2s CW) + 1.8Hz harmonic breathing
///       * Inner Fire Star & Hub Core: High-speed hyperdrive plasma core (1.4s CCW!) + 32Hz electric micro-buzz
///   - 3D Floating Layer Separation: Core floats with elevated Z-parallax depth over the heavy outer anchor rim.
class SudarshanChakra11V7 extends StatefulWidget {
  const SudarshanChakra11V7({
    super.key,
    this.size = 170,
    this.speed = 1.0,
  });
  ...
  @override
  State<SudarshanChakra11V7> createState() => _SudarshanChakra11V7State();
}

class _SudarshanChakra11V7State extends State<SudarshanChakra11V7>
  ...
  late final AnimationController _depthResonanceController;
  ...
  void initState() {
    ...
    // 2. Main Wheel: Medium momentum (8000ms CW)
    _wheelController = AnimationController(
      vsync: this,
      duration: Duration(milliseconds: (8000 / widget.speed).round()),
    )..repeat();

    // 3. Dashed Arc Circle (- - -): Counter buffer (7500ms CCW)
    _dashedRingController = AnimationController(
      vsync: this,
      duration: Duration(milliseconds: (7500 / widget.speed).round()),
    )..repeat();

    // 4. Outer Fire Hexagon Star: Accelerated spin (3200ms CW)
    _fireClockwiseController = AnimationController(
      vsync: this,
      duration: Duration(milliseconds: (3200 / widget.speed).round()),
    )..repeat();

    // 5. Inner Fire Hexagon Star: Hyper-Fast Hyperdrive Core (1400ms CCW!)
    _fireAntiClockwiseController = AnimationController(
      vsync: this,
      duration: Duration(milliseconds: (1400 / widget.speed).round()),
    )..repeat();
    ...
    // 7. Depth Resonance Kinetics Controller (3200ms)
    _depthResonanceController = AnimationController(
      vsync: this,
  ...
  void dispose() {
    ...
    _depthResonanceController.dispose();
    super.dispose();
  }
      ...
      child: Center(
        child: AnimatedBuilder(
          animation: Listenable.merge([
            ...
            _glowController,
            _depthResonanceController,
          ]),
          builder: (context, _) {
            final t = _depthResonanceController.value * 3.2; // t in seconds (0.0 to 3.2)

            // LAYER 0: Outermost 36-Teeth Rim: Slow, heavy gravitational drift (0.7Hz)
            final outerDriftX = math.sin(t * 0.7 * math.pi * 2) * 1.5;
            final outerDriftY = math.cos(t * 0.7 * math.pi * 2) * 1.2;

            // LAYER 1: Main Wheel: Centered gyro resonance (2.4Hz)
            final wheelX = math.sin(t * 2.4 * math.pi * 2) * 0.9;
            final wheelY = math.cos(t * 2.4 * math.pi * 2) * 0.7;

            // LAYER 1B: Dashed Circle: Medium counter shimmer (8.0Hz)
            final dashedX = math.cos(t * 8.0 * math.pi * 2) * 1.1;
            final dashedY = math.sin(t * 8.0 * math.pi * 2) * 0.8;

            // LAYER 2: Outer Fire Star: Radial flare breathing (1.8Hz, +-4.0%)
            final fireOuterScale = 1.0 + (0.040 * math.sin(t * 1.8 * math.pi * 2));
            final fireOuterX = math.sin(t * 1.8 * math.pi * 2) * 1.2;
            final fireOuterY = math.cos(t * 1.8 * math.pi * 2) * 1.0;

            // LAYER 3: Inner Hyperdrive Core: Intense high-speed micro-buzz (32Hz) + elevated floating Z scale (+-2%)
            final coreMicroTremorX = math.sin(t * 32.0 * math.pi * 2) * 1.6;
            final coreMicroTremorY = math.cos(t * 32.0 * math.pi * 2) * 1.3;
            final coreZLevitationScale = 1.0 + (0.025 * math.sin(t * 1.8 * math.pi * 2));
            ...
            return Stack(
              ...
              children: [
                ...
                // 2. 3D Tilt Container with Differential Layer Separation
                Transform(
                  alignment: Alignment.center,
                  transform: Matrix4.identity()
                    ..scale(1.60, 1.60, 1.0)
                  ...
                  child: Stack(
                    ...
                    children: [
                      ...
                      // LAYER 1: Main Sacred Wheel (8s CW + 2.4Hz resonance)
                      Transform(
                        alignment: Alignment.center,
                        transform: Matrix4.identity()..translate(wheelX, wheelY, 0.0),
                        child: Transform.rotate(
                      ...
                      // LAYER 1B: Dashed Circle Line (- - -) (7.5s CCW + 8Hz shimmer)
                      Transform(
                        alignment: Alignment.center,
                      ...
                      // LAYER 2: Outer Fire Star (Fast 3.2s CW + 1.8Hz flare breathing)
                      Transform(
                        alignment: Alignment.center,
                        transform: Matrix4.identity()
                      ...
                      // LAYER 3: Inner Fire Star & Hyper-Fast Hyperdrive Core (Ultra-Fast 1.4s CCW! + 32Hz buzz + floating Z)
                      Transform(
                        alignment: Alignment.center,
                        transform: Matrix4.identity()

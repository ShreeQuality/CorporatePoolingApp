...
/// Screen 1 Emblem: Sacred Animated Sudarshan Chakra (V11-V1: Heartbeat "Lub-Dub" Kinetic Vibration)
/// Pattern:
///   - Double Pulse: Sharp energetic thump (Lub at 0.0 - 0.12) -> Secondary harmonic rebound (Dub at 0.18 - 0.30) -> Serene Rest Gap (0.30 - 1.0)
///   - Total Heartbeat Cycle: 2600ms
...
class SudarshanChakra11V1 extends StatefulWidget {
  const SudarshanChakra11V1({
    super.key,
    this.size = 170,
    this.speed = 1.0,
  });
  ...
  @override
  State<SudarshanChakra11V1> createState() => _SudarshanChakra11V1State();
}

class _SudarshanChakra11V1State extends State<SudarshanChakra11V1>
  ...
  void initState() {
    ...
    // 7. Organic Heartbeat (Lub-Dub) Cycle: 2600ms
      ...
      child: Center(
        child: AnimatedBuilder(
          ...
          builder: (context, _) {
            ...
            // HEARTBEAT PATTERN:
            // 1. "LUB" (Primary Strong Ventricular Systole): 0.00 -> 0.12 (~310ms)
            if (t >= 0.0 && t <= 0.12) {
              final p = t / 0.12;
              final env = math.sin(p * math.pi); // Smooth bell curve
              // Rapid micro-thump vibration
              tremorX = math.sin(p * 7 * math.pi * 2) * 3.6 * env;
              tremorY = math.cos(p * 7 * math.pi * 2) * 2.6 * env;
              heartScale = 1.0 + (0.055 * env); // Expands +5.5%
              auraSurge = 0.14 * env;
            }
            // 2. "DUB" (Secondary Harmonic Atrial Rebound): 0.18 -> 0.30 (~310ms)
            else if (t >= 0.18 && t <= 0.30) {
              final p = (t - 0.18) / 0.12;
              final env = math.sin(p * math.pi);
              tremorX = math.sin(p * 5 * math.pi * 2) * 2.0 * env;
              tremorY = math.cos(p * 5 * math.pi * 2) * 1.5 * env;
              heartScale = 1.0 + (0.028 * env); // Softer rebound +2.8%
              auraSurge = 0.07 * env;
            }
            // 3. DIASTOLE (Serene Resting Phase): 0.30 -> 1.00 (~1820ms) -> Steady continuous spin
            ...
            return Stack(
              ...
              children: [
                // 1. Tilted 3D Ground Shadow (Pulsing organically with heartbeat)
                Transform(
                  alignment: Alignment.center,
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      gradient: RadialGradient(
                        colors: [
                          Color(0xFFFF9D00).withValues(alpha: 0.35 + auraSurge),
                          Color(0xFFFF5500).withValues(alpha: 0.18 + (auraSurge * 0.5)),
                          Colors.transparent,
                        ],
                ...
                // 2. 3D Tilt Container with Heartbeat Thump & Micro-Tremor
                Transform(
                  alignment: Alignment.center,
                  transform: Matrix4.identity()
                    ..scale(1.60 * heartScale, 1.60 * heartScale, 1.0)
                    ..rotateX(-54 * math.pi / 180)
                    ..rotateZ(10 * math.pi / 180),
                  child: Stack(
                    alignment: Alignment.center,

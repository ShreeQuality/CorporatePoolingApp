...
/// Screen 1 Emblem: Sacred Animated Sudarshan Chakra (V11-V10: Smooth Organic Swell + Delicate 3D Micro-Precession)
/// Pattern:
///   - Base Kinetics: Slow gentle cosmic swell & organic exhale from V11-V9 (Zero snap, 4200ms cycle)
///   - Subtle 3D Gyro Precession: Gentle, refined micro-tilt (+-1.2° pitch, +-1.4° yaw, +-0.8° roll)
///   - Velvet Micro-Resonance: 12Hz soft harmonic tremor fading smoothly into tranquil rotation
...
class SudarshanChakra11V10 extends StatefulWidget {
  const SudarshanChakra11V10({
    super.key,
    this.size = 170,
    this.speed = 1.0,
  });
  ...
  @override
  State<SudarshanChakra11V10> createState() => _SudarshanChakra11V10State();
}

class _SudarshanChakra11V10State extends State<SudarshanChakra11V10>
  ...
  late final AnimationController _swellAndGyroController;
  ...
  void initState() {
    ...
    // 7. Slow Swell & Subtle Gyro Micro-Precession Cycle: 4200ms
    _swellAndGyroController = AnimationController(
      vsync: this,
  ...
  void dispose() {
    ...
    _swellAndGyroController.dispose();
    super.dispose();
  }
      ...
      child: Center(
        child: AnimatedBuilder(
          animation: Listenable.merge([
            ...
            _glowController,
            _swellAndGyroController,
          ]),
          builder: (context, _) {
            final t = _swellAndGyroController.value;

            // 1. SUBTLE 3D GYROSCOPIC MICRO-PRECESSION (Delicate & refined)
            final gyroAngleX = (-54.0 + (1.2 * math.sin(t * math.pi * 2))) * math.pi / 180;
            final gyroAngleY = (1.4 * math.cos(t * math.pi * 2)) * math.pi / 180;
            final gyroAngleZ = (10.0 + (0.8 * math.sin((t * math.pi * 2) + (math.pi / 4)))) * math.pi / 180;

            double tremorX = 0, tremorY = 0;
            double dynamicScale = 1.0;
            double auraSurge = 0.0;

            // 2. ORGANIC SWELL KINETICS (from V11-V9, No snap)
            if (t < 0.60) {
              final p = t / 0.60; // 0.0 -> 1.0
              final smoothP = 0.5 * (1.0 - math.cos(p * math.pi));
              dynamicScale = 1.0 - (0.035 * smoothP);

              final freq = 8.0 + (8.0 * p);
              final intensity = 0.25 + (1.2 * math.pow(p, 1.5));
              tremorX = math.sin(p * freq * math.pi * 2) * intensity;
              tremorY = math.cos(p * freq * math.pi * 2) * (intensity * 0.75);
              auraSurge = 0.10 * smoothP;
            } else {
              ...
              final decay = 1.0 - p;
              tremorX = math.sin(p * 10.0 * math.pi * 2) * (1.3 * decay);
              tremorY = math.cos(p * 10.0 * math.pi * 2) * (0.9 * decay);
              auraSurge = 0.10 * swell;
            }
            ...
            return Stack(
              ...
              children: [
                ...
                // 2. 3D Tilt Container with Delicate Gyro Precession + Smooth Swell
                Transform(
                  alignment: Alignment.center,
                  transform: Matrix4.identity()

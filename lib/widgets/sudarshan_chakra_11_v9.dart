...
/// Screen 1 Emblem: Sacred Animated Sudarshan Chakra (V11-V9: Slow Cosmic Energy Swell & Smooth Exhale - No Snap)
/// Pattern:
///   - Phase 1 (Slow Gentle Charge / Inhale): 0.00 -> 0.60 (~2500ms).
///       * Slow, subtle compression (1.0 -> 0.965)
///       * Gentle, velvety ascending micro-tremor (8Hz -> 18Hz) building warmth softly
///   - Phase 2 (Smooth Radiant Exhale / Release): 0.60 -> 1.00 (~1700ms).
///       * Ultra-smooth organic sinusoidal expansion (0.965 -> 1.025 -> 1.00) with ZERO snap
///       * Energy softly dissolves and settles back into serene tranquil rotation
///   - Total Seamless Cycle: 4200ms
...
class SudarshanChakra11V9 extends StatefulWidget {
  const SudarshanChakra11V9({
    super.key,
    this.size = 170,
    this.speed = 1.0,
  });
  ...
  @override
  State<SudarshanChakra11V9> createState() => _SudarshanChakra11V9State();
}

class _SudarshanChakra11V9State extends State<SudarshanChakra11V9>
  ...
  late final AnimationController _energySwellController;
  ...
  void initState() {
    ...
    // 7. Slow Cosmic Swell Cycle: 4200ms (Slow Charge + Smooth Organic Exhale, No Snap)
    _energySwellController = AnimationController(
      vsync: this,
  ...
  void dispose() {
    ...
    _energySwellController.dispose();
    super.dispose();
  }
      ...
      child: Center(
        child: AnimatedBuilder(
          animation: Listenable.merge([
            ...
            _glowController,
            _energySwellController,
          ]),
          builder: (context, _) {
            final t = _energySwellController.value;

            double tremorX = 0, tremorY = 0;
            double dynamicScale = 1.0;
            double auraSurge = 0.0;

            // 1. PHASE 1: SLOW GENTLE CHARGE / INHALE (0.00 -> 0.60, ~2500ms)
            if (t < 0.60) {
              final p = t / 0.60; // 0.0 -> 1.0
              // Smooth inward compression: 1.0 -> 0.965
              final smoothP = 0.5 * (1.0 - math.cos(p * math.pi));
              dynamicScale = 1.0 - (0.035 * smoothP);

              // Velvety progressive tremor (8Hz -> 18Hz, gentle intensity)
              final freq = 8.0 + (10.0 * p);
              final intensity = 0.3 + (1.5 * math.pow(p, 1.5));
              tremorX = math.sin(p * freq * math.pi * 2) * intensity;
              tremorY = math.cos(p * freq * math.pi * 2) * (intensity * 0.75);
              auraSurge = 0.12 * smoothP;
            }
            // 2. PHASE 2: SMOOTH RADIANT EXHALE / NO SNAP (0.60 -> 1.00, ~1700ms)
            else {
              final p = (t - 0.60) / 0.40; // 0.0 -> 1.0
              // Smooth organic sine wave release from 0.965 -> 1.025 -> 1.00 (Zero snap!)
              final swell = math.sin(p * math.pi);
              dynamicScale = 0.965 + (0.035 * p) + (0.025 * swell);

              // Softly fading decay tremor
              final decay = 1.0 - p;
              tremorX = math.sin(p * 12.0 * math.pi * 2) * (1.6 * decay);
              tremorY = math.cos(p * 12.0 * math.pi * 2) * (1.1 * decay);
              auraSurge = 0.12 * swell;
            }

            final baseGlow = _glowController.value;
            final glowScale = 1.62 + (baseGlow * 0.08) + (auraSurge * 0.6);
            ...
            return Stack(
              ...
              children: [
                ...
                // 2. 3D Tilt Container with Smooth Slow Swell Kinetics
                Transform(
                  alignment: Alignment.center,
                  transform: Matrix4.identity()
